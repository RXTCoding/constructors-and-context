...
function Employee(name, email, hireDate) {
  ...
  */
  ...
  function User(name, age,email, savedPosts){ //<-- greyed out posts indicate that parameteres are not being used
    this.name=name
    this.age= age
    this.email= email
  ...
  User.prototype.addSavedPost= function(id, title, rating){
    let addedPost= {
      id: id,
      title: title,
      rating //<-- same as saying "rating: rating"
    }
    this.savedPosts.push(addedPost);
    console.log(this.savedPosts)
  }

  // const newPost= savedPosts (176, 'Aladdin', 5)
  
  ////////// PROBLEM 6 //////////
  
  // You will be using the constructor function you just created in problem 5.
  // Write a prototype method for the User constructor function named removeSavedPost that will take in one number parameter (continued..)
